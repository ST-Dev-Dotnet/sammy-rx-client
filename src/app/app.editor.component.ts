...
$inputListBorder:1px solid #a6a6a6;
...
$inputSwitchSliderOffBgColor:#cccccc;
$inputSwitchHandleOffBgColor:#ffffff;
$inputSwitchSliderOffHoverBgColor:#b7b7b7;
$inputSwitchSliderOffFocusBgColor:#b7b7b7;
$inputSwitchSliderOnBgColor:$primaryColor;
$inputSwitchSliderOnHoverBgColor:$primaryDarkColor;
$inputSwitchHandleOnBgColor:#cccccc;
...
$paginatorElementHoverBgColor:#e0e0e0;
...
$tableHeaderCellHoverIconColor:#333333;
...
$disabledOpacity:.5;                      //opacity of a disabled item
    ...
    initiate() {
        ...
        this.sass.preloadFiles(base, directory, files);
    ...
    compile(event, resetting) {
        ...
        this.sass.compile(this.variables + ';@import "_core"', function(result) {
    ...
    highlightsHtml() {
        let variablesAsHtml = this.variables.replace(/^\$([a-zA-z])*:/gim, u => `<span class='scss-variable'>` + u + `</span>`);
        variablesAsHtml = variablesAsHtml.replace(/\/\/(.*)/gim, u => `<span class='scss-comment'>` + u + `</span>`);
        variablesAsHtml = variablesAsHtml.replace(/#(?:[0-9a-f]{3}){1,2}/gim, u => `<span contenteditable="false" style='width:10px; height: 10px; display: inline-block; border: 1px solid #fff; background-color:` + u + `'></span>` + u);

        return variablesAsHtml;
    }
}
